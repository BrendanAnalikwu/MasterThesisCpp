	...
	for (int q=0;q<ncells;++q)
	  {
	    ...
	    Vertex2d v;
	    v.equ(0.25,M2->vertex2d(ioc[0]),0.25,M2->vertex2d(ioc[1]),0.25,M2->vertex2d(ioc[2]),0.25,M2->vertex2d(ioc[3]));
	...
	for (_iter=1; _iter<=_niter; _iter++)
	  {
	    ...
	    GetMultiLevelSolver()->GetSolver()->CellVisu("Results/dgh",glDGH,_iter+ADAITER*1000);
	    
	    GlobalTimer.stop("--> Transport");
	    
	    //-----------------------------FV--Transport----------------------------
        
	    // set sea ice problem (to solve sea ice momentum equaion)
	    cout<<"Momentum"<<endl;
	    ...
	    //  if(_iter==_niter){
	    GetMultiLevelSolver()->GetSolver()->Visu("Results/v",u,_iter+ADAITER*1000);      
	    // }	    
